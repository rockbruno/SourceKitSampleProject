...
class SourceKit {
    ...
    private static func getToolchainPath() -> String {
        let task = Process()
    ...
    public init() {
        self.path = Self.getToolchainPath()
...
extension SourceKit {
    ...
    public func sendSync(_ req: SKRequestDictionary) -> SKResponseDictionary {
        let resp = SKResponse(api.send_request_sync(req.dict), sourcekitd: self)
        guard let dict = resp.value else {
            preconditionFailure(resp.error!)
        }
        return dict
    }
}
...
public struct sourcekitd_keys {
    ...
    let accessibility: sourcekitd_uid_t
    let kind: sourcekitd_uid_t
    let inheritedtypes: sourcekitd_uid_t
    ...
    init(api: sourcekitd_functions_t) {
        ...
        accessibility = api.uid_get_from_cstr("key.accessibility")!
...
public struct sourcekitd_values {
    ...
    let accessibility_public: sourcekitd_uid_t
    ...
    init(api: sourcekitd_functions_t) {
        ...
        accessibility_public = api.uid_get_from_cstr("source.lang.swift.accessibility.public")!
...
final class SKResponseArray {
    ...
    func get(_ i: Int, parent: SKResponseDictionary! = nil) -> SKResponseDictionary {
